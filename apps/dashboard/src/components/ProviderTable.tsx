...
export function ProviderTable({ providers }: { providers: readonly (Provider & { activeErrorsCount?: number; lastErrorDiagnostic?: any })[] }) {
    ...
    <div className="overflow-x-auto">
      <table className="w-full border-collapse text-sm" style={{ background: 'transparent' }}>
        ...
        <tbody style={{ background: 'transparent' }}>
          {providers.map((p) => {
            const hasError = p.health !== 'healthy' || (p.activeErrorsCount ?? 0) > 0;
            return (
              <tr key={p.id} className="border-b border-white/[0.05]" style={{ background: 'transparent' }}>
                ...
                <td className="px-3 py-2 text-white/80 font-medium">{p.displayName}</td>
                ...
                <td className="px-3 py-2 text-right">
                  <ErrorResolveButton
                    ...
                    errorCount={p.activeErrorsCount ?? (hasError ? 1 : 0)}
                    diagnostic={p.lastErrorDiagnostic}
                    size="xs"
                    variant={hasError ? 'badge' : 'icon'}
                  />
                </td>
              </tr>
            );
