        ...
        <div className="flex items-center gap-2">
          {(keys ?? []).some((k) => k.status === 'cooldown' || k.status === 'exhausted') && (
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm font-semibold text-amber-300 shadow-lg transition hover:scale-[1.02] hover:bg-amber-500/20 active:scale-95"
              title="Reset all keys stuck in cooldown/exhausted state back to active"
            >
              <Zap className="h-4 w-4" /> Reset All Cooldowns
            </button>
          )}
          <button
            onClick={() => setShowAdd((prev) => !prev)}
            className="relative group overflow-hidden rounded-xl bg-gradient-to-r from-nexus-600 to-cyan-600 px-5 py-3 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:scale-[1.02] hover:shadow-nexus-500/25 active:scale-95"
          >
            <span className="relative z-10 flex items-center gap-2">
              <Plus className={`h-4 w-4 transition-transform duration-300 ${showAdd ? 'rotate-45' : ''}`} />
              {showAdd ? 'Close Vault Registration' : 'Register New API Key'}
            </span>
