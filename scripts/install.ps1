...
#   9. Print the dashboard URL and next-steps
#
...
try {
  ...
  # normal install rather than failing the whole install.
...
# 1) Re-link the bin defensively (in case pnpm skipped it earlier).
Push-Location $REPO_DIR
try { pnpm install --silent } finally { Pop-Location }

# 2) Add the repo's .bin directory to the user PATH if not already present.
$userPath  = [Environment]::GetEnvironmentVariable('Path', 'User')
$pathParts = $userPath -split ';' | Where-Object { $_.Trim() -ne '' }
if ($pathParts -notcontains $binDir) {
  $newPath = ($pathParts + $binDir) -join ';'
  [Environment]::SetEnvironmentVariable('Path', $newPath, 'User')
  # Refresh this session's PATH so the verification below sees it.
...
# 3) Verify the command actually resolves before claiming success.
$anxResolved = $false
$anxVersion  = $null
if (Test-Path $anxCmd) {
  try {
