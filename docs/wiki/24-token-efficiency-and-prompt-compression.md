...
4. **Compression Lab (live per-engine preview)**: `POST /v1/compression/pipeline-preview` runs the stacked `compressPipeline` (minify · dedupe · collapse-arrays · elide-middle) on a prompt and returns the **real measured savings per engine** + total. The dashboard exposes this at `/compression`.
