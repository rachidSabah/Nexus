    ...
    steps: [
      {
        ...
        agent: 'claude-code',
        model: 'claude-3-5-sonnet',
        task: 'Run the project build for branch "${inputs.branch}" and report any compile/bundle errors with the exact commands to reproduce them.\n\nRepo: ${inputs.repo}\nBranch: ${inputs.branch}',
        systemPrompt: 'You are a build engineer. Produce a concise build report: success or the failing command + error.',
        maxTokens: 1500,
      },
      {
        name: 'lint',
        agent: 'deepseek-coder',
        model: 'deepseek-coder',
        task: 'Run the linter on branch "${inputs.branch}" and list every warning/error by file:line.\n\nBuild context:\n${build}',
        inputs: ['build'],
        systemPrompt: 'You are a lint engineer. Report only actionable lint issues; ignore auto-fixable noise.',
        maxTokens: 1500,
      },
      {
        name: 'test',
        agent: 'codex-cli',
        model: 'gpt-4o',
        task: 'Run the test suite for branch "${inputs.branch}" and report pass/fail counts, the failing test names, and the first failure trace.\n\nBuild context:\n${build}\n\nLint context:\n${lint}',
        inputs: ['build', 'lint'],
        systemPrompt: 'You are a test engineer. Report results clearly: how many passed/failed and what broke.',
        maxTokens: 2000,
      },
      {
        name: 'package',
        agent: 'mistral-coder',
        model: 'mistral-large',
        task: 'Package the build artifacts for branch "${inputs.branch}" (e.g. docker build, npm pack, or wheel). Report the artifact name/location and any packaging warnings.\n\nBuild context:\n${build}\n\nTest context:\n${test}',
        inputs: ['build', 'test'],
        systemPrompt: 'You are a release engineer. Confirm the artifact is produced and note any warnings.',
        maxTokens: 1500,
      },
      {
        name: 'openPr',
        agent: 'claude-code',
        model: 'claude-3-5-sonnet',
        task: 'Open (or update) a pull request from branch "${inputs.branch}" into "${inputs.baseBranch}" and verify its CI status. Use the title "${inputs.prTitle}" and summarize the build/lint/test results.\n\nBuild:\n${build}\n\nLint:\n${lint}\n\nTest:\n${test}\n\nPackage:\n${package}',
        inputs: ['build', 'lint', 'test', 'package'],
        systemPrompt: 'You are a release engineer. Open the PR, attach the CI summary, and confirm the PR URL + checks.',
        maxTokens: 2000,
      },
    ],
    inputs: [
      { name: 'repo', description: 'The repository to build (e.g. owner/name or local path)', required: true },
      { name: 'branch', description: 'The feature/working branch to build', required: true },
      { name: 'baseBranch', description: 'The base branch the PR targets (default main)', required: false },
      { name: 'prTitle', description: 'Title for the pull request', required: false },
    ],
    outputs: [
      { name: 'buildReport', fromStep: 'build' },
      { name: 'lintReport', fromStep: 'lint' },
      { name: 'testReport', fromStep: 'test' },
      { name: 'packageArtifact', fromStep: 'package' },
      { name: 'pullRequest', fromStep: 'openPr' },
    ],
    tags: ['pr', 'build', 'ci', 'pipeline'],
  },
} as const;
